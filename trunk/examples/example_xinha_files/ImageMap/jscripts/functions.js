...
Xinha._addEvent(window, 'load', init);

function init() {
	
	__dlg_init(null,{width:700,height:850});
	img_obj = window.dialogArguments["img"];
	...
	if (mapname != null && mapname != '') {
		...
		var maps = editor._doc.getElementsByTagName('MAP');
		//console.log(maps);
...
function updateAction() {
	if (img_obj != null && img_obj.nodeName.toLowerCase() == "img") {

		if (typeof map_obj == 'undefined' || map_obj == null) {
			map_obj = editor._doc.createElement('MAP');
